...
$.BootstrapTable = class extends $.BootstrapTable {
  ...
  initTr (item, idx, data, parentDom) {
    ...
    const nodes = data.filter(it => item[this.options.idField] === $.deepFind(it,this.options.parentIdField))
    ...
    for (let i = 0; i <= len; i++) {
      ...
      this.options.rowStyle = (item, idx) => {
        ...
        const pid = $.deepFind(item,this.options.parentIdField)? $.deepFind(item,this.options.parentIdField) : 0
  ...
  initRow (item, idx, data, parentDom) {
    if (this.treeEnable) {
      if (
        this.options.rootParentId === $.deepFind(item,this.options.parentIdField) ||
        !$.deepFind(item,this.options.parentIdField)
      ) {
      ...
      //如果父节点不在列表中 则 level 0 并渲染
      let parentId = $.deepFind(item,this.options.parentIdField)
