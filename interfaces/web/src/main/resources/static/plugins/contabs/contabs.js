...
function scrollToTab(element) {
    ...
    // 可视区域非tab宽度
    var tabOuterWidth = calSumWidth($(".content-tabs").children().not(".J_menuTabs"))+5;
    //可视区域tab宽度
    ...
    if ($(".page-tabs-content").outerWidth() < visibleWidth) {
        scrollVal = 0;
    } else if (marginRightVal <= (visibleWidth - $(element).outerWidth(true) - $(element).next().outerWidth(true) - 10)) {
        if ((visibleWidth - ($(element).next().outerWidth(true)+5)) > marginRightVal) {
            scrollVal = marginLeftVal;
            var tabElement = element;
            while ((scrollVal - $(tabElement).outerWidth() -5) > ($(".page-tabs-content").outerWidth() - visibleWidth)) {
                scrollVal -= ($(tabElement).prev().outerWidth()+5);
                tabElement = $(tabElement).prev();
            }
        }
    } else if (marginLeftVal > (visibleWidth - $(element).outerWidth(true) - $(element).prev().outerWidth(true) - 10)) {
...
function scrollTabLeft() {
    ...
    if ($(".page-tabs-content").width() < visibleWidth) {
        ...
        while ((offsetVal + $(tabElement).outerWidth(true)) <= marginLeftVal) {//找到离当前tab最近的元素
            if (!$(tabElement).next().hasClass("J_menuTab")) {
                break;
            }
...
function closeCurrentTab(){
    ...
    if (othis.hasClass('active')) {
        ...
        if (othis.next('.J_menuTab').length) {
        ...
        if (othis.prev('.J_menuTab').length) {
...
//打开选项卡菜单
function openTab(url,title, isNew,isDelay){//isDelay代表页面点击选项卡的时候才加载  isNew 为true时，打开一个新的选项卡；为false时，如果选项卡不存在，打开一个新的选项卡，如果已经存在，使已经存在的选项卡变为活跃状态
    // 获取标识数据
    var dataUrl = url,
        dataIndex ,
        menuName = title,
    ...
    if(isNew || flag){//isNew为true，打开一个新的选项卡； flag为true，选项卡不存在，打开一个新的选项卡。
        ...
        if(!isDelay) {
            ...
            var str1 = '<iframe class="J_iframe" name="iframe' + dataIndex + '" width="100%" height="100%" src="" frameborder="0" data-id="' + dataUrl + '" seamless></iframe>';
            top.$('.J_mainContent').find('iframe.J_iframe').hide().parents('.J_mainContent').append(str1);
        }
        //显示loading提示
        var index = layer.load(1, {
        ...
        top.$('.J_mainContent iframe:visible').on("load",function(){
            layer.close(index);
        })
        ...
        if(isDelay){
            top.$(".J_menuTab[data-id='"+dataUrl+"']").unbind('click');
            top.$(".J_menuTab[data-id='"+dataUrl+"']").click(function () {
                top.$(".J_iframe[data-id='"+dataUrl+"']").attr("src",dataUrl);
                top.$(".J_menuTab[data-id='"+dataUrl+"']").unbind('click');
            })
        }
    }
...
function go_into(url){
    //找到该选项卡对应的菜单的父菜单
    var $menuLi = top.$("a[data-url='" + top.$("a.J_menuTab.active").data("id") + "']");
    var menuId = $menuLi.attr("id");
    if(url.indexOf("?") > -1){
        url += "&menuId="+menuId;
    }else{
        url += "?menuId="+menuId;
